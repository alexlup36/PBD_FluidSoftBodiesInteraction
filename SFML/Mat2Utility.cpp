...
void JacobiRotate(glm::mat2& A, glm::mat2& R)
{
	if (A[1][0] != 0.0f)
	{
		float d = (A[0][0] - A[1][1]) / (2.0f * A[1][0]);
		...
		for (int k = 0; k < 2; k++)
		{
			float Rkp = c * R[0][k] + s * R[1][k];
			float Rkq = -s * R[0][k] + c * R[1][k];
			R[0][k] = Rkp;
			R[1][k] = Rkq;
		}
	}
...
void PolarDecomposition(const glm::mat2& A, glm::mat2& R, glm::mat2& S)
{
	...
	glm::mat2 s1;
	s1[0][0] = l0 * U[0][0] * U[0][0] + l1 * U[1][0] * U[1][0];
	s1[1][0] = l0 * U[0][0] * U[0][1] + l1 * U[1][0] * U[1][1];
	s1[0][1] = s1[1][0];
	s1[1][1] = l0 * U[0][1] * U[0][1] + l1 * U[1][1] * U[1][1];
